...
static void USB_SetupStage (void)
{
...
static void USB_DataInStage (void)
{
   uint32_t cnt;
...
static void USB_DataOutStage (void)
{
   uint32_t cnt;
...
static void USB_StatusInStage (void)
{
...
static void USB_StatusOutStage (void)
{
...
static uint32_t USB_ReqSetClrFeature (uint32_t sc)
{
   uint32_t n, m;
 ...
 */
 ...
 static uint32_t USB_ReqGetDescriptor (void)
{
   uint8_t *pD;
   uint32_t len, n;
...
static uint32_t USB_ReqSetConfiguration (void)
{
            ...
            while (pD->bLength)
            {
               switch (pD->bDescriptorType)
               {
                  case USB_CONFIGURATION_DESCRIPTOR_TYPE:
                     ...
                     {
                        pD = (USB_COMMON_DESCRIPTOR *) ((uint8_t *) pD +
                                      ((USB_CONFIGURATION_DESCRIPTOR *) pD)
                                          ->wTotalLength);
                        continue;
                     }
...
static uint32_t USB_ReqSetInterface (void)
{
   USB_COMMON_DESCRIPTOR *pD;
...
void USB_EndPoint0 (uint32_t event)
{
               ...
               switch (SetupPacket.bmRequestType.BM.Recipient)
               {
                  ...
                  case REQUEST_TO_INTERFACE:
                     ...
                     { /* IF number correct? */
                        switch (SetupPacket.bRequest)
                        {
                           case HID_REQUEST_GET_REPORT:
                              ...
                              {
                                 EP0Data.pData =
                                     EP0Buf; /* point to data to be sent */
                        ...
                        switch (SetupPacket.bRequest)
                        {
                           case MSC_REQUEST_RESET:
                              if ((SetupPacket.wValue.W ==
                                   0) && /* RESET with invalid parameters ->
                                            STALL */
                           ...
                           case MSC_REQUEST_GET_MAX_LUN:
                              if ((SetupPacket.wValue.W ==
                                   0) && /* GET_MAX_LUN with invalid parameters
                                            -> STALL */
                     ...
                     if ((SetupPacket.wIndex.WB.L ==
                          USB_ADC_CIF_NUM) || /* IF number correct? */
   ...
   #if USB_CDC
                     if ((SetupPacket.wIndex.WB.L ==
                          USB_CDC_CIF_NUM) || /* IF number correct? */
